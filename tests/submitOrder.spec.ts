...
test.describe('submit order tests', () => {
  ...
  test('should submit order with 3 product', async ({ page }) => {
    await homePage.selectSingleProduct('0');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('1');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('2');
    await productPage.addAndProceed();
    await cartPage.proceedToCheckout();
    await checkoutPage.fillAddressForm('John', 'Doe', 'Doe Inc', '123456', '123 Main St', 'Apt 1', 'Miami', '33101', 'United States', '1234567890');
    await checkoutPage.fillShippingMethod();
  ...
  test('should not submit order with 3 product because there is not too much stock', async ({ page }) => {
    await homePage.selectSingleProduct('0');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('1');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('2');
    await productPage.changeProductQuantity('123345678');
    await productPage.addAndProceed();
    const messageShown = await productPage.checkStockMessage();
    expect(messageShown).toBeTruthy();
  });

  test('should submit order with 2 product and 2 units of an item', async ({ page }) => {
    await homePage.selectSingleProduct('0');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('1');
    await productPage.addAndContinueShopping();
    await homePage.selectSingleProduct('2');
    await productPage.addAndProceed();
    await cartPage.deleteIcon('1');
    await cartPage.changeQuantitySingleProduct('0', '2');
    await cartPage.proceedToCheckout();
    await checkoutPage.fillAddressForm('John', 'Doe', 'Doe Inc', '123456', '123 Main St', 'Apt 1', 'Miami', '33101', 'United States', '1234567890');
    await checkoutPage.fillShippingMethod();
    await checkoutPage.fillPaymentMethod();
